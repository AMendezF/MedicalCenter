	...
	public String mostrarAgendaMatinal(String fecha) throws SQLException {
		...
		for (String especialidad : especialidades) {
			...
			if (resultSet.next()) {
				...
				agenda += "\t No hay citas para esta fecha" + "\n";
			}
		}
...
//                }
	...
	/**
	 * Devuelve una lista de médicos disponibles para el turno y la especialidad
	 * especificados.
	 *
	 * @param turno Mañana o tarde.
	 * @param especialidad Ginecología, Neurología...
	 * @return Array de tuplas String [(nColegiado1, nombre1), (nColegiado2,
	 * nombre2), (nColegiado3, nombre3)]
	 * @throws SQLException
	 */
	public String[][] getMedicoEspecifico(String turno, String especialidad)
			throws SQLException {
		String sql;
		ResultSet rs;
		// Preparamos consulta para obtener los médicos que buscamos
		sql = "SELECT N_colegiado, Nombre, Apellidos FROM centromedico.medico"
				+ " WHERE Horario='" + turno + "' AND Especialidad=(SELECT "
				+ "Cod_especialidad FROM centromedico.especialidad "
				+ "WHERE Nombre='" + especialidad + "');";
		rs = conexion.makeQuery(sql);
		//Cargamos los médicos obtenidos en una lista
		ArrayList<String[]> listaMedicos = new ArrayList<>();
		String[] medico = new String[2];
		while (rs.next()) {
			medico[0] = String.valueOf(rs.getInt("N_colegiado"));
			medico[1] = rs.getString("Nombre") + " " + rs.getString("Apellidos");
			listaMedicos.add(medico);
		}
		// Parseamos de lista a array de Strings
		String[][] medicosDisponibles = new String[listaMedicos.size()][2];
		for (int i = 0; i < medicosDisponibles.length; i++) {
			for (int j = 0; j < medicosDisponibles[0].length; j++) {
				medicosDisponibles[i][j] = listaMedicos.get(i)[j];
			}
		}
